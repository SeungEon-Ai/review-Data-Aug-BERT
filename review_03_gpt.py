###############################################################################
#
#   리뷰 데이터 BERT — ④ GPT-4o-mini 4배수 증강 학습
#
...
#
#   결과: review_result_gpt.json (Google Drive에 저장)
...
gpt_path = f"{DRIVE_FOLDER}/review_gpt4omini_augmented.csv"
gpt_data = pd.read_csv(gpt_path, encoding="utf-8-sig")
print(f"  원본: {len(all_texts)}건 / GPT 증강: {len(gpt_data)}건")
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(train_val_texts, train_val_labels)):
    ...
    # ★ train에만 GPT 증강 ★
    aug_texts, aug_labels = get_gpt_augmented(train_idx.tolist())
    tr_texts = tr_texts + aug_texts
    tr_labels = tr_labels + aug_labels

    if fold_idx == 0:
        print(f"    원본 train: {len(train_idx)}건 + GPT: {len(aug_texts)}건 = 총 {len(tr_texts)}건")
    ...
    with open(f"{DRIVE_FOLDER}/review_result_gpt_interim.json", "w") as f:
...
aug_texts, aug_labels = get_gpt_augmented(list(range(len(train_val_texts))))
tr_texts = tr_texts + aug_texts
tr_labels = tr_labels + aug_labels
...
with open(f"{DRIVE_FOLDER}/review_result_gpt.json", "w") as f:
...
print(f"  저장: {DRIVE_FOLDER}/review_result_gpt.json")
